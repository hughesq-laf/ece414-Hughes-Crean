...
int main()
{
    ...
    if (mpu6050_begin(&mpu6050))
    {
        ...
        mpu6050_set_accelerometer_measuring(&mpu6050, true);            //USED

        // Enable free fall, motion and zero motion interrupt flags
        mpu6050_set_int_free_fall(&mpu6050, false);
        mpu6050_set_int_motion(&mpu6050, false);                        //MAYBE USED
        mpu6050_set_int_zero_motion(&mpu6050, false);                   //MAYBE USED

        // Set motion detection threshold and duration
        mpu6050_set_motion_detection_threshold(&mpu6050, 2);            //USED MAYBE ALTERED
        mpu6050_set_motion_detection_duration(&mpu6050, 5);             //USED MAYBE ALTERED

        // Set zero motion detection threshold and duration
        mpu6050_set_zero_motion_detection_threshold(&mpu6050, 4);       //USED MAYBE ALTERED
        mpu6050_set_zero_motion_detection_duration(&mpu6050, 2);        //USED MAYBE ALTERED
